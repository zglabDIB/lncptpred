...
df=pd.read_csv('../dataset/uploaded_final_protein.txt',sep='\t')
df=df[df.Protein_Name==prot_name]
...
Final=pd.merge(gen_df,hydropathy_df,how='inner',on='Protein_Name')
Final=pd.merge(Final,chemical_df,how='inner',on='Protein_Name')
Final=pd.merge(Final,volume_df,how='inner',on='Protein_Name')
